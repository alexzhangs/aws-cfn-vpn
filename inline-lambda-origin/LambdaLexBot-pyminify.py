...
def change_ip(intent_request):
    ...
    G=boto3.resource('sns');H=G.Topic(D);H.publish(Message='change_ip');return close(A[_A],'Fulfilled',{_K:_M,_L:'The IP address of instance {} will be replaced with a new one.'.format(B)})
